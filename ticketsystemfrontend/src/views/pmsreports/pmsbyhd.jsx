...
export default function PMSbyHD({ filterType, showChart = true, onDataReady, location }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // const ticketRes = await axios.get(`${config.baseApi}/pmsticket/get-all-pmsticket`);
                const ticketRes = await supabase.from('pmsticket_master').select('*');
                // const noteRes = await axios.get(`${config.baseApi}/authentication/get-all-pmsnotes`);
                const noteRes = await supabase.from('pmsnotes_master').select('*');
                ...
                const datasets = alluser.map(user => {
                    ...
                    const noteIds = [
                        ...new Set(notes.filter(n => n.created_by === name).map(n => n.pmsticket_id))
                    ];
                    ...
                    const worked = allTickets.filter(ticket => {
                        const collab = ticket.assigned_collaborators
                            ?.split(",").map(s => s.trim()) || [];
                        return (
                            noteIds.includes(ticket.pmsticket_id) ||
                ...
                <Bar
                    ...
                    options={{
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'PMS Tickets of Helpdesk Users',
                            }
                        },
