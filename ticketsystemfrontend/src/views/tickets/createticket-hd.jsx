...
export default function CreateTicketHD() {
    ...
    const saveCustomSubCategory = async (category, subCategory) => {
        ...
        try {
            const empInfo = JSON.parse(localStorage.getItem('user')); // fix: was out of scope
            const { error: insertError } = await supabase.from('option_master').insert({
                category: category,
            ...
            if (insertError) {
                console.error('option_master error:', insertError.message, insertError.details, insertError.hint);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data: dataid } = await supabase.from('ticket_master').select('*');
            const ticket_id_length = dataid.length;
            const ticket_id = ticket_id_length + 1;

            // 2. Insert ticket
            const { data: datac, error: insertError } = await supabase
                .from('ticket_master')
                .insert({
                    ticket_id: ticket_id,
            ...
            console.log('Ticket created:', datac);
