...
export default function ViewTicket() {
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            try {
                ...
                const getTicket = await supabase.from('notes_master')
                    .select('*')
        ...
        const interval = setInterval(() => {
            const getUpdated = async () => {
                try {
                    ...
                    const response = await supabase.from('ticket_master').select('*').eq('ticket_id', ticket_id).single();
    ...
    const handleReview = async (value, userfeedback) => {
        ...
        try {
            ...
            const res = await supabase.from('review_master').select('*');
            ...
            if (feedTicket.length > 0) {
                ...
                await supabase.from('review_master').delete().eq('ticket_id', ticket_id);
                //add a new review 
                ...
                //     score
                // });
                await supabase.from('review_master').insert({
                    ticket_id: ticket_id,
                    review: userfeedback,
                    user_id: hdUser.user_id,
                    created_by: empInfo.user_name,
                ...
                await supabase.from('ticket_master').update({ is_reviewed: true }).eq('ticket_id', ticket_id);
    ...
    //                 await supabase.from('ticket_master').insert({
    //                     created_at: new Date(),
    //                     notes: `${currentUserData.user_name} re opened the ticket.`,
    ...
    //             await supabase.from('ticket_master').update({ is_notifiedhd: true }).eq('ticket_id', ticket_id);
    ...
    const handleSave = async () => {
        try {
            ...
            // ✅ Already migrated
            const fetchticket = await supabase
                .from('ticket_master')
                .select('*')
                .eq('ticket_id', ticket_id)
                .single();

            const ticket = fetchticket.data;

            if (ticket.is_locked === false || ticket.updating_by === empInfo.user_name || ticket.updating_by === null) {

                // Check changed fields for logs
                const changedFields = [];
                const fieldsToCheck = ['ticket_subject', 'tag_id', 'ticket_status', 'ticket_urgencyLevel', 'ticket_category', 'ticket_SubCategory', 'Description', 'Attachments'];
                fieldsToCheck.forEach(field => {
                    const original = originalData[field];
                ...
                // ✅ Handle file attachments via Supabase Storage
                let attachmentPaths = formData.Attachments || '';

                if (formData.attachmentFiles && formData.attachmentFiles.length > 0) {
                    const uploadPromises = formData.attachmentFiles.map(async (file) => {
                        const filePath = `attachments/${ticket_id}/${Date.now()}_${file.name}`;
                ...
                // ✅ Replace axios update-ticket with supabase update
                ...
                // ✅ Fix: was inserting into ticket_master but likely meant ticket_notes
                // Also fixed typo: ticket_i → ticket_id
                if (formData.ticket_status === 're-opened') {
                    const { error: noteError } = await supabase
                        .from('notes_master') // 🔁 Replace with your actual notes table
                        .insert({
                            created_at: new Date(),
                            notes: `${currentUserData.user_name} re opened the ticket.`,
                            created_by: currentUserData.user_name,
                            ticket_id: ticket_id, // ✅ Fixed typo from ticket_i
                        });
    ...
    const renderAttachment = () => {
        ...
        const filePaths = formData.Attachments.split(',').filter(p => p.trim() !== '');
        ...
        return (
            <div className="d-flex flex-column">
                {filePaths.map((filePath, idx) => {
                    ...
                    // ✅ Use Supabase getPublicUrl to get the correct viewable URL
    ...
    const renderNoteAttachments = (attachmentPath) => {
        ...
        const filePaths = attachmentPath.split(',').filter(p => p.trim() !== '');
        ...
        return (
            <div className="mt-2">
                ...
                {filePaths.map((filePath, idx) => {
                    const fileUrl = filePath.trim(); // ✅ already a full URL
