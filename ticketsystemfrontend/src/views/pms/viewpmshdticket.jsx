...
export default function ViewHDPmsTicket() {
    ...
    useEffect(() => {
        ...
        const fetchNotes = async () => {
            ...
            try {
                ...
                const response = await supabase.from('pmsnotes_master')
                    .select('*')
                ...
                const allfeedback = await supabase.from('pmsreview_master').select('*').eq('pmsticket_id', pmsticket_id)
                    .order('created_at', { ascending: true })
    ...
    const handleNotifyReview = async () => {
        ...
        try {
            ...
            //SEND EMAIL ( NOT WORKING SINCE MIGRATED TO SUPABASE)
        ...
        try {
            ...
            if (ticket.is_locked === "0" || ticket.locked_by === empInfo.user_name || ticket.locked_by === null) {
                ...
                if (formData.pms_status === 'closed') {
                    ...
                    // await knex('pmsticket_master').where('pmsticket_id', pmsticket_id)
                    await supabase.from('pmsticket_master').update({
                        assigned_to: empInfo.user_name,
                        updated_at: new Date(),
                        updated_by: empInfo.user_name,
    ...
    useEffect(() => {
        if (!pmsticket_id || !empInfo?.user_name) return;
        ...
        const lockAsset = async () => {
            try {
                ...
                if (asset.is_locked === "1" && asset.locked_by && asset.locked_by !== currentUser) {
                    if (isMounted) {
                        setLockError(`This ticket is being edited by ${asset.locked_by}`);
                        setLockModal(true);
        ...
        const checkLockStatus = async () => {
            try {
                ...
                if (isMounted) {
                    ...
                    if (asset.is_locked === "1" && asset.locked_by && asset.locked_by !== currentUser) {
                        setLockModal(true);
                        setLockError(`This asset is being edited by ${asset.locked_by}`);

                    }
                    // If current user has the lock or no lock
                    else if (asset.is_locked === "0" || asset.locked_by === currentUser) {
                        setLockModal(false);
        ...
        const unlockAsset = async () => {
            try {
                await supabase
                    ...
                    .eq('pmsticket_id', pmsticket_id)
                    .eq('locked_by', currentUser); // Only unlock if current user owns the lock
            } catch (err) {
    ...
    }, [pmsticket_id, empInfo?.user_name]);
    // //once page was open it will check if assets is lock or no
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await supabase.from('pmsticket_master').select('*').eq('pmsticket_id', pmsticket_id).single();

                const assets = Array.isArray(res.data) ? res.data[0] : res.data;

                if (assets.is_locked === '0' || assets.locked_by === empInfo.user_name || assets.locked_by === null) {
                    setLockModal(false)
                }
                else {
                    setLockModal(true)
    ...
    const HandleResolution = async (e) => {
        ...
        try {
            ...
            await supabase.from('pmsnotes_master').insert({
                note: 'HelpDesk Resolved the ticket: \n' + resolution,
                created_by: empInfo.user_name,
                created_at: new Date(),
                pmsticket_id: pmsticket_id,
            })
            ...
            await supabase.from('pmstat_master').insert({
                tat: turnaroundtime,
                user_id: empInfo.user_id,
                pmsticket_id: pmsticket_id,
                user_name: empInfo.user_name,
    ...
    const handleSave = async () => {
        try {
            ...
            if (ticket.is_locked === '0' || ticket.locked_by === empInfo.user_name || ticket.locked_by === null) {
                ...
                //     params: { user_name: formData.pmsticket_for }
                // })
                const changedTicketFor = await supabase.from('users_master').select('*').eq('user_name', formData.pmsticket_for).single();
                ...
                if (formData.pmsticket_for) {
                    ...
                    if (formData.pms_status === 'open') {
                        ...
                        console.log(`Ticket ${pmsticket_id} assigned to ${formData.assigned_to}`);
                    }
                ...
                if (formData.pms_status === 'open') {
                    if (!formData.assigned_to || formData.assigned_to.trim() === '') {
                        ...
                        await supabase.from('pmsticket_master').update({
                            ...
                            updated_by: empInfo.user_name,
                            tag_id: formData.tag_id
                        }).eq('pmsticket_id', formData.pmsticket_id)
                        ...
                        await supabase.from('pmsticket_logs').insert({
                            ...
                            created_by: empInfo.user_name,
                            created_at: new Date(),
                            changes_made: `${empInfo.user_name} assinged the ticket to ${hdUser.user_name}`
                        })
            ...
            if (ticket.is_locked === '0' || ticket.locked_by === empInfo.user_name || ticket.locked_by === null) {
                ...
                try {
                    ...
                    await supabase.from('pmsticket_logs').insert({
                        ...
                        created_by: empInfo.user_name,
                        created_at: new Date(),
                        changes_made: `${empInfo.user_name} Archived this ticket`
                    });
    ...
    const UnArchive = async () => {
        ...
        try {
            ...
            await supabase.from('pmsticket_logs').insert({
                ...
                created_by: empInfo.user_name,
                created_at: new Date(),
                changes_made: `${empInfo.user_name} Un-archived this ticket`
            });
