    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            try {
                ...
                await supabase.from('pmsticket_master').select('*')
                    .then((res) => {
                        if (user.emp_tier === 'user') {
                            const userTickets = res.data.filter(
                                (pmsticket) => pmsticket.pmsticket_for === userName && pmsticket.is_active === true &&
                                    (pmsticket.is_reviewed !== true || pmsticket.is_reviewed === null)
                            );
                            ...
                            const userTickets = res.data.filter(
                                (ticket) =>
                                    ...
                                    (ticket.is_reviewed !== true || ticket.is_reviewed === null) &&
                                    ticket.pms_status !== 'closed'
                            );
                            console.log('llllllllllllllllllllllllllllllll   ', userName)
                            setAllTicket(userTickets);
                        }
                    })
        ...
        if (prevStat === 'open' && selectedNewStatus === 'assigned') {
            ...
            try {
                await supabase.from('pmsticket_master').update({
                    ...
                    updated_at: new Date()
                }).eq('pmsticket_id', selectedTicket.pmsticket_id)

                await supabase.from('pmsticket_logs').insert({
                    pmsticket_id: selectedTicket.pmsticket_id,
                    tag_id: selectedTicket.tag_id,
                    changes_made: `${empInfo.user_name} updated status from assigned to ${selectedNewStatus}`,
            ...
            try {
                ...
                await supabase.from('pmsticket_logs').insert({
                    pmsticket_id: selectedTicket.pmsticket_id,
                    tag_id: selectedTicket.tag_id,
                    changes_made: `${empInfo.user_name} change the status from assigned to open`,
                    created_at: new Date(),
                    created_by: empInfo.user_name
                })
            ...
            try {
                ...
                await supabase.from('pmsticket_logs').insert({
                    pmsticket_id: selectedTicket.pmsticket_id,
                    tag_id: selectedTicket.tag_id,
                    changes_made: `${empInfo.user_name} change the status from assigned to in-progress`,
                    created_at: new Date(),
                    created_by: empInfo.user_name
                })
        ...
        ////TAT
            ...
            try {
                ...
                await supabase.from('pmsticket_logs').insert({
                    pmsticket_id: selectedTicket.pmsticket_id,
                    tag_id: selectedTicket.tag_id,
                    changes_made: `${empInfo.user_name} change the status from in-progress to open`,
                    created_at: new Date(),
                    created_by: empInfo.user_name
                })
            ...
            try {
                ...
                await supabase.from('pmsticket_logs').insert({
                    pmsticket_id: selectedTicket.pmsticket_id,
                    tag_id: selectedTicket.tag_id,
                    changes_made: `${empInfo.user_name} change the status from in-progress to assigned`,
                    created_at: new Date(),
                    created_by: empInfo.user_name
                })
                setSuccessful(`Succesfully changed Ticket ID: ${selectedTicket.pmsticket_id} to open!`);
