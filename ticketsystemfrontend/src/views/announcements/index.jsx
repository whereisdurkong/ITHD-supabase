...
export default function Announcements() {
    ...
    useEffect(() => {
        const fetchAnnouncements = async () => {
            try {
                ...
                const res = await supabase.from('announcements_master').select('*');
                ...
                const getUsernames = await supabase.from('users_master').select('*').eq('user_name', JSON.stringify(usernames))
    ...
    const HandleSave = async () => {
        ...
        try {
            ...
            if (error) {
                ...
                const announcement_id = data[0].announcement_id;

                // Now insert into logs
                await supabase.from('announcements_logs').insert({
                    announcement_id: announcement_id,
                    changes_made: `${empInfo.user_name} has added an announcement.`,
                    created_at: new Date(),
                    created_by: empInfo.user_name,
                });
            }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await supabase.from('announcements_master').update({
                announcements: announcementText,
                updated_by: empInfo.user_name,
                announcementTitle: announcementTitleText,
                updated_at: new Date()
            }).eq('announcement_id', editId)
    ...
    const handleDelete = async (announcementId) => {
        ...
        try {
            ...
            setAnnouncementsList(announcementsList.filter(item => item.announcement_id !== announcementId));
            setAncId(null);
                ...
                <div className="mt-4">
                    ...
                    {announcementsList.length === 0 ? (
                        ...
                        currentAnnouncements.map((item) => (
                            <Card key={item.announcement_id} className="mb-4 shadow-sm" style={{ borderRadius: '15px' }}>
                                <Card.Body>
                                    <div className="d-flex align-items-center justify-content-between mb-4">
                                        ...
                                        {access && (
                                            <div onClick={() => {
                                                setAncId(item.announcement_id === ancId ? null : item.announcement_id);
                                            }}>
                                                <i className="bi bi-three-dots" style={{ cursor: 'pointer' }}></i>

                                                {ancId === item.announcement_id && (
                                                    <div
                                                    ...
                                                    >
                                                        ...
                                                        <div
                                                            ...
                                                            style={{ cursor: 'pointer' }}
                                                            onClick={() => {
                                                                handleDelete(item.announcement_id);
                                                            }}
                                                        >
                                                            Archive
                                                        </div>
