...
export default function NetworkArchive() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const all = await supabase.from('knowledgebase_master').select('*');
    ...
    const handleUnArchive = async (id) => {
        try {
            ...
            await supabase.from('knowledgebase_master').update({
                is_active: 1,
    ...
    const handleDelete = async (id) => {
        ...
        if (window.confirm("Are you sure you want to delete ?")) {
            try {
                ...
                await supabase.from('knowledgebase_master').delete().eq('kb_id', id);
    ...
    const handleSave = async () => {
        ...
        if (newTitle === '') {
            ...
            try {
                ...
                await supabase.from('knowledgebase_master').insert({
                    kb_title: newTitle,
                    kb_desc: newContent,
                    created_by: empInfo.user_name,
                    kb_category: "network",
                    ...
                    <Accordion defaultActiveKey={0} flush>
                        {faqData.length === 0 ? (
                            <div className="text-center text-muted py-3">
                                No archived network articles found
                            </div>
                        ) : (
