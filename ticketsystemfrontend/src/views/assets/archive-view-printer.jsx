...
export default function ArchiveViewPrinter() {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                const lmd = data.filter((pc) => pc.assigned_location === "lmd" && pc.pms_category === "printer" && pc.is_active === '0');
                setLmdpc(lmd);

                const corp = data.filter((pc) => pc.assigned_location === "corp" && pc.pms_category === "printer" && pc.is_active === '0');
                setCorppc(corp);
            } catch (err) {
