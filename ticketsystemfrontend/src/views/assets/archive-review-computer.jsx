...
export default function ArchiveReviewComputer() {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await supabase.from('pms_logs').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: `${currentUser} deleted this asset ${tag_id}`,
                created_at: new Date(),
                created_by: currentUser
            })
    ...
    const handleArchive = async () => {
        ...
        try {
            ...
            // })
            await supabase.from('pms_master').update({ is_active: 1 }).eq('pms_id', pms_id);

            await supabase.from('pms_logs').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: `${currentUser} un-archived this asset`,
                created_by: currentUser,
                created_at: new Date()
            })
