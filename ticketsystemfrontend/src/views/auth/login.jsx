  ...
  const Auth = async (e) => {
    ...
    try {
      ...
      if (error) {
        ...
        setLoginError('Login Error', 'Database error occurred');
        setLoading(false);
        return false;
      }
      ...
      if (data && data.length > 0) {
        ...
        setLoginError('Login Error', 'Invalid username or password. Please try again.');
        setLoading(false);
      }

    } catch (err) {
      console.error('Login error:', err);
      setLoginError('Connection Error', 'Unable to connect to server. Please check your internet or try again later.');
      setLoading(false);
    }
  };
