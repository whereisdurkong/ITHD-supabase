...
export default function UsersView() {
    ...
    useEffect(() => {
        ...
        const fetchUserInfo = async () => {
            try {
                ...
                const response = await supabase.from('users_master').select('*').eq('user_id', user_id).single();
    ...
    const UpdateForm = async (e) => {
        ...
        try {
            ...
            const response = await supabase.from('users_master').update(updatedUser).eq('user_id', user_id);
            await supabase.from('users_logs').insert({
                user_id: user_id,
                changes_made: `${current_user.user_name} updated the account of ${username}`,
                created_at: new Date(),
    ...
    const DeleteUser = async () => {
        ...
        try {
            ...
            // });
            await supabase.from('users_master').delete().eq('user_id', user_id).single();
            await supabase.from('users_logs').insert({
                user_id: user_id,
                changes_made: `${current_user.user_name} deleted the account of ${username}`,
                created_at: new Date(),
                created_by: current_user.user_name
            })
    ...
    const handleChangePassword = async (e) => {
        ...
        try {
            ...
            // })
            await supabase.from('users_master').update({ pass_word: newPassword }).eq('user_id', user_id);
            await supabase.from('users_logs').insert({
                user_id: user_id,
                changes_made: `${current_user.user_name} changed password of ${username}`,
                created_at: new Date(),
                created_by: current_user.user_name
            })
