...
export default function CreatePMSUser() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await supabase.from('pmsticket_master').select('*');
            const pmsticket_id_length = data.length;
            const pmsticket_id = pmsticket_id_length + 1;


            const response = await supabase.from('pmsticket_master').insert({
                pmsticket_id: pmsticket_id,
                pms_status: 'open',
                pmsticket_for: empInfo.user_name,
                assigned_location: empInfo.emp_location,
                tag_id: formData.tag_id,
                description: formData.Description,
                created_by: empInfo.user_name,
            ...
            await supabase.from('pmsticket_logs').insert({
                pmsticket_id: pmsticket_id,
                tag_id: formData.tag_id,
                changes_made: `${empInfo.user_name} created this pms ticket`,
                created_by: empInfo.user_name,
                created_at: new Date(),
            })
