    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const ticketRes = await supabase.from('ticket_master').select('*')
                // const notesRes = await axios.get(`${config.baseApi}/authentication/get-all-notes`);
                const notesRes = await supabase.from('notes_master').select('*');
                ...
                const a = await supabase.from('users_master').select('*').eq('user_name', calluser)
