...
export default function ViewPrinter() {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                const res = await supabase.from('pms_master').select('*').eq('pms_category', 'printer')
                const data = res.data || [];
                ...
                const lmd = data.filter((pc) => pc.assigned_location === "lmd" && pc.pms_category === "printer" && pc.is_active === '1');
                setLmdpc(lmd);

                const corp = data.filter((pc) => pc.assigned_location === "corp" && pc.pms_category === "printer" && pc.is_active === '1');
                setCorppc(corp);

                // Extract unique departments from all active printers
    ...
    const filteredPCs = displayedPCs.filter((pc) => {
        ...
        const matchesSearch = (
            ...
            pc.model?.toLowerCase().includes(search) ||
            ...
            <Row className="align-items-center mb-3 g-2">
                <Col xs={12}>
                    <div className="d-flex align-items-center gap-2 flex-wrap">
                        <Form.Control
                            type="text"
                            placeholder="Search by Assigned To, Tag ID, Department, Model, or Created By"
                            value={searchTerm}
                        ...
                        <Button onClick={() => navigate('/assets-add-printer')}>
                            + Add Printer
                        </Button>
                    </div>
                </Col>
