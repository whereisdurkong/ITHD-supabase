...
export default function ReviewLaptop() {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                const res = await supabase.from('pms_master').select('*').eq('pms_id', pms_id).single();
                const data = res.data;
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                const res = await supabase.from('users_master').select('*').eq('user_name', currentUser).single();
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            await supabase.from('pms_master').update({
                tag_id: tag_id,
            ...
            await supabase.from('pms_logs').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: changeSummary,
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // });
            // await supabase.from('pms_master').delete().eq('pms_id', pms_id);

            await supabase.from('pms_logs').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: `${currentUser} deleted this asset`,
                created_by: currentUser,
                created_at: new Date()
            });
    ...
    const handleArchive = async () => {
        ...
        try {
            setLoading(true);
            // await axios.post(`${config.baseApi}/pms/archive-device`, {
            //     pms_id,
            ...
            await supabase.from('pms_logs'.insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: `${currentUser} archived this asset`,
                created_by: currentUser,
                created_at: new Date()
            }))
    ...
    const HandleLogs = async () => {
        // const res = await axios.get(`${config.baseApi}/pms/get-computer-by-id`, { params: { pms_id } });
        const res = await supabase.from('pms_master').select('*').eq('pms_id', pms_id);
        const asset = res.data;
        ...
        const lockAsset = async () => {
            try {
                const { data: asset, error } = await supabase
                    .from('pms_master')
                    .select('is_lock, lock_by')
                    .eq('pms_id', currentTicketId)
                    .single();

                if (error) throw error;

                // If locked by someone else
                if (asset.is_lock === "1" && asset.lock_by && asset.lock_by !== currentUser) {
                    if (isMounted) {
                        setLockError(`This asset is being edited by ${asset.lock_by}`);
                        setLockModal(true);
                        setClose(false);
                    }
                    return;
        ...
        const checkLockStatus = async () => {
            try {
                ...
                if (isMounted) {
                    ...
                    else if (asset.is_lock === "0" || asset.lock_by === currentUser) {
                        setLockModal(false);
                        setClose(true);
                    }
                }
        ...
        const unlockAsset = async () => {
            try {
                await supabase
                    ...
                    .eq('pms_id', currentTicketId)
                    .eq('lock_by', currentUser); // Only unlock if current user owns the lock
            } catch (err) {
    ...
    return (
        <Container fluid className="pt-100" style={{ background: 'linear-gradient(to bottom, #ffe798ff, #b8860b)', minHeight: '100vh', paddingTop: '100px' }}>
                        ...
                        <Card className="p-4 shadow-sm">
                            <div
                                ...
                                <div className="text-start">
                                    <h4 style={{ margin: 0 }}>Review Laptop</h4>
                                    <h6
                                        title='asset logs'
                                        style={{
                                            margin: 0,
            ...
            <Modal show={showArchiveModal} onHide={() => setShowArchiveModal(false)} centered>
                ...
                <Modal.Body>
                    Are you sure you want to archive this Laptop?
                </Modal.Body>
                <Modal.Footer>
