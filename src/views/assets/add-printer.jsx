...
export default function AddPrinter() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await supabase.from('pms_master').select('*');
            const pms_id_length = data.length;
            const pms_id = pms_id_length + 1;


            await supabase.from('pms_master').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                department: department,
                assign_to: assign_to,
            ...
            await supabase.from('pms_logs').insert({
                pms_id: pms_id,
                tag_id: tag_id,
                changes_made: `${currentUser} added new printer ${tag_id}`,
                created_at: new Date(),
                created_by: currentUser
            })
