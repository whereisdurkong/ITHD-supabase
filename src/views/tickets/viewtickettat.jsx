...
export default function ViewTicketTAT({ ticket_id }) {
    ...
    useEffect(() => {
        try {
            const fetchData = async () => {
                ...
                const res = await supabase.from('tat_master').select('*');
                const resData = res.data || [];

                const tatsupport = resData.filter(a => a.ticket_type === 'support');
                const ticketTAT = tatsupport.filter(a => a.ticket_id === ticket_id);
    ...
    return (
        ...
        <Card>
            ...
            {tat.length === 0 ? (
                <p className="text-muted mb-0">No logs available for this ticket.</p>
            ) : (
                <div
                    style={{
