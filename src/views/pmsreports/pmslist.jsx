        ...
        const fetchData = async () => {
            try {
                const [ticketRes, assetRes] = await Promise.all([
                    ...
                    await supabase.from('pms_master').select('*')
                ...
                const userDetailsPromises = Array.from(allUsernames).map(async (username) => {
                    try {
                        ...
                        // });
                        const userRes = await supabase.from('users_master').select('*').eq('users_name', username);
                ...
                const notesPromises = closedTickets.map(async (ticket) => {
                    try {
                        ...
                        const noteRes = await supabase.from('pmsnotes_master').select('*').eq('pmsticket_id', ticket.pmsticket_id);
                        return {
